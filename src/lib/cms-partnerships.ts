...
export async function getPartnershipsPage(): Promise<PartnershipsPageData> {
  ...
  try {
    ...
    const global = await payload.findGlobal({ slug: 'partnerships-page' })
    ...
    return {
      ...
      getStarted: {
        ...
        imageUrl: String(gs.imageUrl ?? fallback.getStarted.imageUrl),
        imageAlt: String(gs.imageAlt ?? fallback.getStarted.imageAlt),
